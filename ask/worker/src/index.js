...
const TOOLS = [
  ...
  { name: "read_data", description: "Read part of a data file. `path` is a dotted/indexed path into the JSON, e.g. 'skill.ericeira.meteofrance_wave.3' or 'quality'; omit for the top-level keys. Output is truncated to ~6 KB, so narrow the path.", input_schema: { type: "object", properties: { source: { type: "string" }, path: { type: "string" } }, required: ["source"] } },
...
3. If no tool can answer (the question is not about this site, or the site has no such data), reply exactly: I can't answer that from this site. — and one sentence on what the site does have.
4. Be brief: a direct answer, then the sources (file paths or page URLs) on one line starting with "Sources:".
...
async function index(env) {
  ...
  const r = await fetch(`${env.SITE}/ask/index.json`, { cf: { cacheTtl: 600 } });
  INDEX = await r.json();
  return INDEX;
}

function searchTexts(idx, query) {
  const terms = fold(query).match(/[a-z0-9]{3,}/g) || [];
...
async function runTool(env, idx, name, input) {
  ...
  if (name === "read_data") {
    ...
    const r = await fetch(`${env.SITE}/${input.source}`, { cf: { cacheTtl: 300 } });
    if (!r.ok) return JSON.stringify({ error: `fetch ${r.status}` });
    const v = dig(await r.json(), input.path || "");
    return v === undefined ? JSON.stringify({ error: "path not found" }) : clip(v);
  }
...
async function ask(env, question, ip) {
  const t0 = Date.now();
  ...
  if (c.hits >= Number(env.PER_IP_PER_HOUR)) return { declined: "rate", answer: `That is ${env.PER_IP_PER_HOUR} questions in an hour from this address; try again later.`, trace: {} };
  ...
  for (let turn = 0; turn < Number(env.MAX_TOOL_CALLS) + 1; turn++) {
    ...
    if (!toolUses.length || trace.tools.length >= Number(env.MAX_TOOL_CALLS)) { answer = text; stopped = toolUses.length ? "tool_cap" : res.stop_reason; break; }
  ...
  trace.protocol = { numbers_without_tool: trace.has_digits && !trace.numbers_from_tools && !trace.declined, unverified_quote: /["“][^"”]{12,}["”]/.test(answer) && trace.quotes.verified === 0 };
